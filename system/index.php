...
try {
	...
	if(!is_file($decoratorPath)){
		...
		$smarty->registerPlugin("function","dec_include", "smarty_plugin_func_dec_include");
...
function smarty_plugin_func_dec_include($parameters,$smarty){
	...
	if(!empty($parameters['path'])){
		...
		if(strlen($path)<4 || strtolower(substr($path, -4))!='.tpl'){
			$path.='.tpl';
		}
		try {
			ob_start();
